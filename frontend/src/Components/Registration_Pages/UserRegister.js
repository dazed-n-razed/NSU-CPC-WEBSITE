...
const UserRegister = () => {
    ...
    uploadTask.on(
      ...
      () => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          setInputs((prev) => {
            const updatedInputs = { ...prev, [fileType]: downloadURL };
            // Call uploadtodb after setting the inputs
            uploadtodb();
  ...
  const submitHandler = () => {
    ...
    // Skip Firebase upload for now - just proceed to uploadtodb
    // Profile picture will be set to default in the backend if not provided
    uploadtodb();
    return true;
  };
  ...
  const uploadtodb = async () => {
    try {
      console.log("Uploading to DB with data:", {
        name,
        status,
        nsuID,
        email,
